...
#define INPUT_WIDTH   64 //from 512
#define INPUT_HEIGHT  64 //from 512 
#define OPTIMIZER "Adam"
#define LEARNING_RATE 0.05f
#define REPLAY_MEMORY 10000
#define BATCH_SIZE 128 //from 8
...
#define INTERIM_REWARD  3.0f
#define INTERIM_REWARD_OFFSET 0.5f
#define ALPHA 0.4f
#define TRAIL1 false
...
bool ArmPlugin::createAgent()
{
	...
	*/
	...
	//agent = NULL;
	uint32_t NUM_ACTIONS = 6; //3 joints with 2 possible states (increase and decrease)
...
void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
{
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		*/
		...
		bool collisionWithTube = false;
		//check if there is collision with the tube
		if (TRAIL1){
			...
			if( strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0 ){
				collisionWithTube = true;
			}
			if( strcmp(contacts->contact(i).collision2().c_str(), COLLISION_ITEM) == 0 ){
				collisionWithTube = true;
			}
			if(collisionWithTube){
...
void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
{
	...
	if( hadNewState && !newReward )
	{
		...
		/*
		/ TODO - set appropriate Reward for robot hitting the grou 4.0nd.
		/
		*/
		...
		*/ 
		...
		if(!checkGroundContact)
		{
			...
			if( episodeFrames > 1 )
			{
				...
				//rewardHistory = REWARD_WIN / (1 + avgGoalDelta) / 4.0 - 2.0 ;
				rewardHistory =  avgGoalDelta * INTERIM_REWARD - INTERIM_REWARD_OFFSET;
				newReward     = true;
			
				//printf("distGoal is %0.2f, avgGoalDelta is %0.2f\n", distGoal, avgGoalDelta);
			}
